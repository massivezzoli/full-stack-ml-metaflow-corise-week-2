...
def labeling_function(row):
    """Labelling function from wk1"""
    return 1 if row['rating']>=4 else 0

def clean_data(df):
    # transformations
    df.columns = ["_".join(name.lower().strip().split()) for name in df.columns]
    df_ = df.loc[~df.review_text.isna(), :].copy()
    df_["review"] = df_["review_text"].astype("str")
    _has_review_df = df_[df_["review_text"] != "nan"]
    reviews = _has_review_df["review_text"]
    labels = _has_review_df.apply(labeling_function, axis=1)
    df_ = pd.DataFrame({"label": labels, **_has_review_df})


    _df = pd.DataFrame({"review": reviews, "label": labels})
    return _df, df_

def model_score(val_df, pred_col):
    """Score accuracy and ROC-AUC"""
    acc = accuracy_score(val_df['label'], val_df[pred_col])
    roc = roc_auc_score(val_df['label'], val_df[pred_col])
    print(f"ACC score: {acc:.3f}, ROC-AUC score: {roc:.3f}")
    return acc, roc
...
class BaselineChallenge(FlowSpec):
    ...
    def baseline(self):
        ...
        most_common_class = self.traindf['label'].value_counts(sort=True, ascending=False).index[0]
        self.valdf[self._name] = most_common_class
        acc, rocauc = model_score(self.valdf, self._name)
    ...
    def model(self):
        ...
        self.hyperparam_set = [{"vocab_sz": 100}, {"vocab_sz": 300}, {"vocab_sz": 500}, {"vocab_sz": 600}]
        pathspec = f"{current.flow_name}/{current.run_id}/{current.step_name}/{current.task_id}"
    ...
    @step
    def end(self):
        print("BaselineChallenge Completed")
